...
#ifdef ZCL_OTA
ota_callBack_t tuyaFree_otaCb =
{
	tuyaFree_otaProcessMsgHandler,
};
#endif
 ...
 * FUNCTIONS
 */
s32 tuyaFree_bdbNetworkSteerStart(void *arg){
...
s32 tuyaFree_bdbFindAndBindStart(void *arg){
...
s32 tuyaFree_rejoinBacckoff(void *arg){
...
//	printf("bdbInitCb: sta = %x, joined = %x\n", status, joinedNetwork);
	...
	if(status == BDB_INIT_STATUS_SUCCESS){
		...
		if(joinedNetwork){
			...
			steerTimerEvt = TL_ZB_TIMER_SCHEDULE(tuyaFree_bdbNetworkSteerStart, NULL, jitter);
		}
	}else{
		if(joinedNetwork){
//			zb_rejoinReqWithBackOff(zb_apsChannelMaskGet(), g_bdbAttrs.scanDuration);
			if(!switchRejoinBackoffTimerEvt){
				switchRejoinBackoffTimerEvt = TL_ZB_TIMER_SCHEDULE(tuyaFree_rejoinBacckoff, NULL, 60 * 1000);
			}
		}
	}
}
	...
	switch(status){
		case BDB_COMMISSION_STA_SUCCESS:
			...
			if(!g_switchAppCtx.bdbFBTimerEvt){
				g_switchAppCtx.bdbFBTimerEvt = TL_ZB_TIMER_SCHEDULE(tuyaFree_bdbFindAndBindStart, NULL, 50);
			}
#endif
			break;
...
extern void tuyaFree_zclIdentifyCmdHandler(u8 endpoint, u16 srcAddr, u16 identifyTime);
void zbdemo_bdbIdentifyCb(u8 endpoint, u16 srcAddr, u16 identifyTime){
#if FIND_AND_BIND_SUPPORT
	tuyaFree_zclIdentifyCmdHandler(endpoint, srcAddr, identifyTime);
#endif
}
...
void zbdemo_bdbFindBindSuccessCb(findBindDst_t *pDstInfo){
	...
	zcl_identify_identifyCmd(TUYA_FREE_ENDPOINT, &dstEpInfo, FALSE, 0, 0);
#endif
}
...
#ifdef ZCL_OTA
void tuyaFree_otaProcessMsgHandler(u8 evt, u8 status)
{
	//printf("tuyaFree_otaProcessMsgHandler: status = %x\n", status);
 ...
 * @return  None
 */
void tuyaFree_leaveCnfHandler(nlme_leave_cnf_t *pLeaveCnf)
{
 ...
 * @return  None
 */
void tuyaFree_leaveIndHandler(nlme_leave_ind_t *pLeaveInd)
{
    //printf("tuyaFree_leaveIndHandler, rejoin = %d\n", pLeaveInd->rejoin);
